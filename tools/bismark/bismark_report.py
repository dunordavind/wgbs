...
class BismarkReportMerger:
    def __init__(self, reports_dir):
        self.__reports_dir = reports_dir if reports_dir.endswith('/') else reports_dir + '/'

    def reports_dir(self):
        return self.__reports_dir

    def merge_reports(self) -> BismarkReport:
        bismark_reports = self.list_report_files()
        return sum(bismark_reports)

    def list_report_files(self):
        files = glob.glob(self.reports_dir() + "*.txt")
        bismark_report_parsers = [BismarkReportParser(file_path) for file_path in files]
        return [BismarkReport.fromparser(bismark_report_parser) for bismark_report_parser
                in bismark_report_parsers
                if bismark_report_parser.is_valid()]


class TestBismarkReportAnalyzer(unittest.TestCase):
    def setUp(self):
        self.splitted_reports_dir = "TestData/SplittedBismarkReports"
        merger = BismarkReportMerger(self.splitted_reports_dir)
        self.merged_report = merger.merge_reports()
        self.valid_merged_report = BismarkReport.fromfile("TestData/valid_merged_bismark_report.txt")

    def test_that_total_pairs_analyzed_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_pairs_analyzed,
                         self.merged_report.total_pairs_analyzed)
    
    def test_that_number_unique_matches_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.number_unique_matches,
                         self.merged_report.number_unique_matches)
    
    def test_that_num_of_unaligned_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_unaligned,
                         self.merged_report.num_of_unaligned)
    
    def test_that_num_of_crossmapped_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_crossmapped,
                         self.merged_report.num_of_crossmapped)
    
    def test_that_num_of_nonextractable_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_nonextractable,
                         self.merged_report.num_of_nonextractable)
    
    def test_that_num_of_top_strand_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_top_strand,
                         self.merged_report.num_of_top_strand)
    
    def test_that_num_of_compl_to_top_strand_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_compl_to_top_strand,
                         self.merged_report.num_of_compl_to_top_strand)
    
    def test_that_num_of_compl_to_bottom_strand_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_compl_to_bottom_strand,
                         self.merged_report.num_of_compl_to_bottom_strand)
    
    def test_that_num_of_bottom_strand_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.num_of_bottom_strand,
                         self.merged_report.num_of_bottom_strand)
    
    def test_that_total_cs_analysed_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_cs_analysed,
                         self.merged_report.total_cs_analysed)
    
    def test_that_total_met_cs_cpg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_met_cs_cpg_context,
                         self.merged_report.total_met_cs_cpg_context)
    
    def test_that_total_met_cs_chg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_met_cs_chg_context,
                         self.merged_report.total_met_cs_chg_context)
    
    def test_that_total_met_cs_chh_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_met_cs_chh_context,
                         self.merged_report.total_met_cs_chh_context)
    
    def test_that_total_met_cs_unknown_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_met_cs_unknown_context,
                         self.merged_report.total_met_cs_unknown_context)
    
    def test_that_total_unmet_cs_cpg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_unmet_cs_cpg_context,
                         self.merged_report.total_unmet_cs_cpg_context)
    
    def test_that_total_unmet_cs_chg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_unmet_cs_chg_context,
                         self.merged_report.total_unmet_cs_chg_context)
    
    def test_that_total_unmet_cs_chh_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_unmet_cs_chh_context,
                         self.merged_report.total_unmet_cs_chh_context)
    
    def test_that_total_unmet_cs_unknown_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.total_unmet_cs_unknown_context,
                         self.merged_report.total_unmet_cs_unknown_context)
    
    def test_that_met_unmet_cs_ratio_cpg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.met_unmet_cs_ratio_cpg_context,
                         self.merged_report.met_unmet_cs_ratio_cpg_context)
    
    def test_that_met_unmet_cs_ratio_chg_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.met_unmet_cs_ratio_chg_context,
                         self.merged_report.met_unmet_cs_ratio_chg_context)
    
    def test_that_met_unmet_cs_ratio_chh_context_correctly_merged(self):
        self.assertEqual(self.valid_merged_report.met_unmet_cs_ratio_chh_context,
                         self.merged_report.met_unmet_cs_ratio_chh_context)
